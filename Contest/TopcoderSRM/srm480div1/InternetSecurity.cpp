...
{
    bool is[60] = {0};
    vector <string> keys[60];
    vector <string> ans;
    for(int i = 0; i < SZ(keyword); ++ i) {
        string tmp = "";
        for(int j = 0; j < SZ(keyword[i]); ++ j) {
            if(keyword[i][j] == ' ') {
...
//    }
    ...
    while(1) {
        flag = false;
        for(int i = 0; i < SZ(address); ++ i) {
            int num = 0;
            vector <string> tmpd;
            for(int j = 0; j < SZ(keys[i]); ++ j) {
                bool tmpf = false;
                for(int k = 0; k < SZ(dangerous); ++ k) {
                    if(keys[i][j] == dangerous[k])
                        ++ num, tmpf = true;
                }
                if(!tmpf)    tmpd.push_back(keys[i][j]);
            }
            if(num >= threshold && !is[i]) {
                for(int j = 0; j < SZ(tmpd); ++ j)  dangerous.push_back(tmpd[j]);
                flag = true;
                is[i] = true;
            }
        }
    ...
    for(int i = 0; i < SZ(address); ++ i) {
        if(is[i])   ans.push_back(address[i]);
    }
    return ans;
}
